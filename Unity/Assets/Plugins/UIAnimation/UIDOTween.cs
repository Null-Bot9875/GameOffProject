...
namespace GameUtil
{
    public class UIDOTween : MonoBehaviour
    {
        ...
        public SingleTween[] StartSingleTweens = Array.Empty<SingleTween>();
        public SingleTween[] CloseSingleTweens = Array.Empty<SingleTween>();
        ...
        public Sequence DoStartTween(Action action)
        {
            RecoverStatus();
            Status = TweenStatus.Start;
            return DoTweenInternal(StartSingleTweens, OnStartBeforeAnim, OnStartAfterAnim, action);
        }

        public Sequence DoCloseTween(Action action)
        {
            Status = TweenStatus.Close;
            return DoTweenInternal(CloseSingleTweens, OnCloseBeforeAnim, OnCloseAfterAnim, action);
        }

        [ContextMenu("DoStartTween")]
        public Sequence DoStartTween()
        {
            return DoStartTween(null);
        }

        [ContextMenu("DoCloseTween")]
        public Sequence DoCloseTween()
        {
            return DoCloseTween(null);
        }

        public void PlayTween(SingleTween[] tween)
        {
            DoTweenInternal(tween, null, null, null);
        }

        private Sequence DoTweenInternal(SingleTween[] tweens, UnityEvent beforeEvent, UnityEvent afterEvent,
            Action action)
        {
                ...
                foreach (var tween in tweens)
                {
                    switch (tween.AddItemType)
                    {
                        case SingleTween.ItemType.Tweener:
                            ...
                            switch (tween.ItemLinkType)
                            {
                                ...
                                case SingleTween.LinkType.Insert:
                                    ...
                                    break;
                                default:
                                    Debug.LogError("LinkType does not contain an enumeration of this type: " +
                                                   (int)tween.ItemLinkType);
                                    break;
                            }
                        ...
                        case SingleTween.ItemType.Callback:
                            ...
                            break;
                        default:
                            Debug.LogError("ItemType does not contain an enumeration of this type: " +
                                           (int)tween.AddItemType);
                            break;
                    }
                }
